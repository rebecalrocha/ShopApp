...
import { deleteCart, sendMessage } from '../actions'
...
class Checkout extends Component {
  constructor (props) {
    ...
    handleChangeMask = event => {
      if (event.target.name === 'cardNumber') { this.setState({ cardNumber: cardMask(event.target.value) }) }
      if (event.target.name === 'cardDate') { this.setState({ cardDate: dateMask(event.target.value) }) }
      if (event.target.name === 'secCode') { this.setState({ secCode: secMask(event.target.value) }) }
        ...
        <div className='container mt-4'>
          <div className='row sidebar mr-1'>
            ...
            <div className='col-md-8 order-md-1 '>
              ...
              <hr className='mb-4 mx-3' />
              <button style={{ width: '96%', marginBottom: '1em' }} onClick={this.handleOnSubmit} type='submit' className=' mx-3 btn btn-primary btn-lg btn-block'>Place your order</button>
            </div>
